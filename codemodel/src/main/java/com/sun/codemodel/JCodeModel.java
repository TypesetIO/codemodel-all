 ...
 * given time (which can be often accesesd by the <tt>owner()</tt> method.)
 *
 ...
 */
public final class JCodeModel implements Serializable{


	/**
	 * 
	 */
	private static final long serialVersionUID = 476408636425882930L;
	...
	public JDefinedClass _class(int mods, String fullyqualifiedName, ClassType t) throws JClassAlreadyExistsException {
		...
		if (idx < 0)
			...
			return _package(fullyqualifiedName.substring(0, idx))._class(mods, fullyqualifiedName.substring(idx + 1),
					t);
	}
	
	public void addClass(String fullyqualifiedName, JDefinedClass jDefinedClass) throws JClassAlreadyExistsException {
		int idx = fullyqualifiedName.lastIndexOf('.');
		if (idx < 0)
			rootPackage().addClass(fullyqualifiedName.substring(idx + 1), jDefinedClass);
		else
			_package(fullyqualifiedName.substring(0, idx)).addClass(fullyqualifiedName.substring(idx + 1), jDefinedClass);
	}
	 ...
	 * to JCodeModel.
	 */
	private class JReferencedClass extends JClass implements JDeclaration, Serializable {
		/**
		 * 
		 */
		private static final long serialVersionUID = -7260977925535868728L;
	 ...
	 * its boxed type (such as <tt>Integer.class</tt>)
	 */
