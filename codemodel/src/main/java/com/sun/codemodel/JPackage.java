...
public final class JPackage
		implements JDeclaration, JGenerable, JClassContainer, JAnnotatable, Comparable<JPackage>, JDocCommentable, Serializable {
	 ...
	 * 
	 * This field is non-null only on Windows, to detect "Foo" and "foo" as a
	 * collision.
	 */
	...
	public void addClass(String name, JDefinedClass jDefinedClass) throws JClassAlreadyExistsException {
		if (upperCaseClassMap != null) {
			JDefinedClass dc = upperCaseClassMap.get(name.toUpperCase());
			if (dc != null)
				throw new JClassAlreadyExistsException(dc);
			upperCaseClassMap.put(name.toUpperCase(), jDefinedClass);
		}
		classes.put(name, jDefinedClass);
	}
	 ...
	 * 
	 * @param name
	 *            Name of the annotation Type declaration to be added to this
	 *            package
